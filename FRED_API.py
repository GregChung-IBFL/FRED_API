...
class Application :
    ...
    def initialize(self) :
        ...
        try:
            with open( API_KEY_FILE_NAME, "r") as file:
                ...
                # NOTE: Validate the key only contains valid characters, etc.
    ...
    def make_api_request( self, request_url ) :
        """Makes a GET request to the API at @request_url.
        ...
        url_with_key = request_url + "&" + self.API_KEY_PARAMETER
        ...
        try:
            ...
            logging.error( F'Exception occurred in API call to "{request_url}":\n{str(e)}' )

        return None

    def make_caching_request( self, request_url ) :
        """Gets the response for the @request_url, from the cache if avaiable, else
        from the API host.

        The response for @request_url may be returned from the cache, if found and not
        ...
        data = self.APICacher.read_cached_data( request_url )
        if not data :
            ...
            data = self.make_api_request( request_url )
            self.APICacher.write_cached_data( request_url, data )
    ...
    def get_child_categories( self, parent_category_id ) :
        """Gets a list of child categories under the specified @parent_category_id.

        Returns the categories directly under the @parent_category_id, as a dictionary
        containing summary info and a list of dictionaries representing the child categories.
        """
        url = F'category/children?category_id={parent_category_id}'
        return self.get_rest_data( url )

    def get_children_series( self, parent_category_id ) :
        """Gets a list of child series under the specified @parent_category_id.
        ...
        Returns the series directly under the @parent_category_id, as a dictionary.
        containing summary info and a list of dictionaries representing the child series.
        """
        url = "category/series?category_id={category_id}&limit={limit}".format( category_id = parent_category_id, limit = self.CONFIG["FRED_API"]["LIMIT_SERIES"] )
        return self.get_rest_data( url )


    def process_series( self, series: dict, category_id: str, tree ) :
        """Process a data series by appending the @series info into the @tree
        as a child of the tree node identified with @category_id.
        ...
        nodeIdentifier = "{category_id}/{seriesID}".format( category_id = str(category_id), seriesID = str(series["id"]) )
        tree.create_node( text, nodeIdentifier, parent = str(category_id) )
        ...
        # NOTE:  Many series contain copyright data requiring approval to use.  To determine this,
    ...
    def process_category( self, depth, category_dict, tree ) :
        ...
        @category_dict must contain values for id and name.  Other values are not used.
        ...
        """
        def process_children_series( self, children_series, category_id, tree ) :
            ...
            if num_series and seriess :
                ...
                for series in seriess[ : self.CONFIG["FRED_API"]["LIMIT_SERIES"]] :
                    self.process_series( series, category_id, tree )
                    num_remaining -= 1

                if num_remaining > 0 :
                    text = F"plus {num_remaining} additional series..."
                    tree.create_node( text, parent = str(category_id) )
            else :
                num_series = 0
        ...
        text = "{indent}[#{category}] {name}".format(indent = " " * (4 * depth), name = category_name, category = category_id)
        logging.info( text )

        # On errors, this can return an empty dict, so be prepared to handle it.
        children_series = self.get_children_series( category_id )
        (num_series, num_descendent_series) = process_children_series( self, children_series, category_id, tree )

        # On errors, this can return an empty dict, so be prepared to handle it.
        children_categories = self.get_child_categories( category_id )
        ...
        if categories :
            ...
            for subdict in categories :
                ...
                tree.create_node( text, str(subdict["id"]), parent = str(category_id) )

                num_descendents = self.process_category( depth = depth+1, category_dict = subdict, tree = tree )
        ...
        cat_node = tree.get_node( str(category_id) )
    ...
    def initialize_tree( self ) :
        ...
        new_tree = Tree()
        new_tree.create_node( "FRED® Services API at " + self.CONFIG["FRED_API"]["BASE_URL"], identifier = str(self.CONFIG["FRED_API"]["ROOT_CATEGORY"]) )
        return new_tree
    ...
    def main() :
        ...
        root_category = app.CONFIG["FRED_API"]["ROOT_CATEGORY"]
        root_category_dict = { "name" : "<root>", "id" : root_category }

        logging.info( F"Beginning DFS traversal of API data layout from root category #{root_category}:" )
        app.process_category( depth = 0, category_dict = root_category_dict, tree = app.fred_data_tree )

        app.write_tree_file( app.fred_data_tree )

        app.APICacher.save_cache_file()

        app_stats = app.get_stats()
        cache_stats = app.APICacher.get_stats()
        logging.debug( F"API Stats: {str(app_stats)}" )
        logging.debug( F"Cacher Stats: {str(cache_stats)}" )
